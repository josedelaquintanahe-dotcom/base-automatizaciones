...
$workflowUrl = "$supabaseBaseUrl/rest/v1/ejecuciones_workflows?select=workflow_name,status,correlation_id,input_payload,created_at,finished_at&workflow_name=eq.automation_client_health_snapshot&correlation_id=eq.$correlationId&order=created_at.desc&limit=1"
