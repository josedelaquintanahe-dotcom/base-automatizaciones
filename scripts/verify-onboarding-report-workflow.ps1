...
$resolvedWebhookUrl = Resolve-WorkflowWebhookUrl -ExplicitUrl $WorkflowWebhookUrl -WorkflowPath "onboarding_sanitized_report_compiled"
...
$execUrl = "$supabaseBaseUrl/rest/v1/ejecuciones_workflows?select=correlation_id,workflow_name,status,started_at,finished_at,input_payload&workflow_name=eq.onboarding_sanitized_report_compiled&correlation_id=eq.$CorrelationId&order=created_at.desc&limit=1"
