...
$systemStatusResponse = Invoke-JsonRequest -Url $statusUrl

if ($systemStatusResponse.Status -ne 200 -or -not $systemStatusResponse.Body) {
  throw "No se pudo obtener un snapshot valido de /api/system/status."
}

$resolvedWebhookUrl = Resolve-WorkflowWebhookUrl -ExplicitUrl $WorkflowWebhookUrl -WorkflowPath "onboarding_dispatch_health_checked"

$payload = @{
  correlation_id = $CorrelationId
  cliente_id = $ClientId
  event_name = "onboarding_activated"
  system_status_snapshot = $systemStatusResponse.Body
} | ConvertTo-Json -Depth 8 -Compress

$invocation = Invoke-WebhookJsonRequest -Url $resolvedWebhookUrl -Body $payload

$supabaseBaseUrl = $env:SUPABASE_URL.TrimEnd("/")
$execUrl = "$supabaseBaseUrl/rest/v1/ejecuciones_workflows?select=correlation_id,workflow_name,status,started_at,finished_at,input_payload&workflow_name=eq.onboarding_dispatch_health_checked&correlation_id=eq.$CorrelationId&order=created_at.desc&limit=1"
