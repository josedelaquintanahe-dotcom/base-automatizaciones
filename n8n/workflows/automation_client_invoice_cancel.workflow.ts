...
const webhookTrigger = trigger({
  ...
  version: 2.1,
  config: {
    name: 'Webhook - automation_client_invoice_cancel',
    parameters: {
      httpMethod: 'POST',
      path: 'automation_client_invoice_cancel',
...
const cancelReason = requestedPayload.cancel_reason ?? null;
...
if (payload.workflow_name && payload.workflow_name !== 'automation_client_invoice_cancel') {
  errors.push('workflow_name invalido para automation_client_invoice_cancel');
}
...
return [
  {
    json: {
      ...
      cancel_reason: typeof cancelReason === 'string' && cancelReason.trim() ? cancelReason.trim() : null,
    }
  }
];
      `,
...
const targetStatus = rollbackMode ? 'pendiente' : 'cancelada';
const invoiceMonth = source.invoice_month ?? invoice?.mes ?? null;
const cancelReason = source.cancel_reason;
...
const rollbackPlan = [
  ...
  'Resolver manualmente cualquier estado pagada o cancelada fuera del flujo previsto',
];

let finalStatus = 'completed';
let actionMode = rollbackMode ? 'rollback' : 'cancel_invoice';
let resultSummary = rollbackMode
  ? 'Factura restaurada a pendiente'
  : 'Factura cancelada de forma controlada';
let nextAction = rollbackMode
  ? 'La factura vuelve a estado pendiente y puede revisarse antes de otro intento'
  : 'Confirmar si la incidencia queda cerrada o si debe regenerarse un nuevo borrador';
...
if (finalStatus === 'completed' && !rollbackMode && previousStatus !== source.expected_status) {
  finalStatus = 'error';
  resultSummary = 'La factura no esta en el estado esperado y no se cancela';
...
if (finalStatus === 'completed' && rollbackMode && previousStatus !== 'cancelada') {
  finalStatus = 'error';
  resultSummary = 'La factura no esta cancelada y no se puede restaurar a pendiente';
...
const insertExecution = node({
  ...
  config: {
    ...
    parameters: {
      ...
      fieldsUi: {
        fieldValues: [
          { fieldId: 'workflow_name', fieldValue: 'automation_client_invoice_cancel' },
...
const respondOk = node({
  ...
  config: {
    ...
    parameters: {
      respondWith: 'json',
      responseBody:
        '={{ ({ accepted: true, workflow_name: "automation_client_invoice_cancel", correlation_id: $json.correlation_id, automation_id: $json.automation_id, result_summary: $json.result_summary, next_action: $json.next_action, sanitized_result: $json.sanitized_result }) }}',
...
const respondError = node({
  ...
  config: {
    ...
    parameters: {
      respondWith: 'json',
      responseBody:
        '={{ ({ accepted: false, workflow_name: "automation_client_invoice_cancel", correlation_id: $json.correlation_id, automation_id: $json.automation_id, result_summary: $json.result_summary, next_action: $json.next_action, sanitized_result: $json.sanitized_result }) }}',
...
export default workflow(
  'automation-client-invoice-cancel-v1',
