...
const webhookTrigger = trigger({
  ...
  version: 2.1,
  config: {
    name: 'Webhook - automation_client_invoice_void_paid',
    parameters: {
      httpMethod: 'POST',
      path: 'automation_client_invoice_void_paid',
...
const expectedStatus = requestedPayload.expected_status ?? 'pagada';
const rawInvoiceMonth = requestedPayload.invoice_month ?? null;
const voidReason = requestedPayload.void_reason ?? null;
...
if (payload.workflow_name && payload.workflow_name !== 'automation_client_invoice_void_paid') {
  errors.push('workflow_name invalido para automation_client_invoice_void_paid');
}
...
if (!rollbackMode && expectedStatus !== 'pagada') {
  errors.push('request_payload.expected_status debe ser pagada en modo normal');
}
...
return [
  {
    json: {
      ...
      void_reason: typeof voidReason === 'string' && voidReason.trim() ? voidReason.trim() : null,
    }
  }
];
      `,
...
const targetStatus = rollbackMode ? 'pagada' : 'cancelada';
const invoiceMonth = source.invoice_month ?? invoice?.mes ?? null;
const voidReason = source.void_reason;

const rollbackPlan = [
  'Usar rollback_mode para restaurar la misma factura al estado pagada',
  'Conservar correlation_id y trazabilidad en ejecuciones_workflows',
  'Resolver manualmente cualquier estado pendiente o cancelada fuera del flujo previsto',
];

let finalStatus = 'completed';
let actionMode = rollbackMode ? 'rollback' : 'void_paid_invoice';
let resultSummary = rollbackMode
  ? 'Factura restaurada a pagada'
  : 'Factura pagada anulada de forma controlada';
let nextAction = rollbackMode
  ? 'La factura vuelve a estado pagada y puede continuar su seguimiento habitual'
  : 'Confirmar si la anulacion resuelve la incidencia o si debe regenerarse el flujo de cobro';
...
if (finalStatus === 'completed' && !rollbackMode && previousStatus !== source.expected_status) {
  finalStatus = 'error';
  resultSummary = 'La factura no esta en el estado esperado y no se anula';
  nextAction = 'Resolver manualmente el estado actual antes de reintentar';
  statusTransitionAllowed = false;
}

if (finalStatus === 'completed' && rollbackMode && previousStatus !== 'cancelada') {
  finalStatus = 'error';
  resultSummary = 'La factura no esta cancelada y no se puede restaurar a pagada';
...
const insertExecution = node({
  ...
  config: {
    ...
    parameters: {
      ...
      fieldsUi: {
        fieldValues: [
          { fieldId: 'workflow_name', fieldValue: 'automation_client_invoice_void_paid' },
...
const respondOk = node({
  ...
  config: {
    ...
    parameters: {
      respondWith: 'json',
      responseBody:
        '={{ ({ accepted: true, workflow_name: "automation_client_invoice_void_paid", correlation_id: $json.correlation_id, automation_id: $json.automation_id, result_summary: $json.result_summary, next_action: $json.next_action, sanitized_result: $json.sanitized_result }) }}',
...
const respondError = node({
  ...
  config: {
    ...
    parameters: {
      respondWith: 'json',
      responseBody:
        '={{ ({ accepted: false, workflow_name: "automation_client_invoice_void_paid", correlation_id: $json.correlation_id, automation_id: $json.automation_id, result_summary: $json.result_summary, next_action: $json.next_action, sanitized_result: $json.sanitized_result }) }}',
...
export default workflow(
  'automation-client-invoice-void-paid-v1',
