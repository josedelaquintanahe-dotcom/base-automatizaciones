...
function Resolve-TraceWebhookUrl {
  ...
  if ($publicBaseUrl.EndsWith("/webhook")) {
    return "$publicBaseUrl/onboarding_trace_verified"
  }

  return "$publicBaseUrl/webhook/onboarding_trace_verified"
}
...
$resolvedWebhookUrl = Resolve-TraceWebhookUrl -ExplicitUrl $WorkflowWebhookUrl
...
$execUrl = "$supabaseBaseUrl/rest/v1/ejecuciones_workflows?select=correlation_id,workflow_name,status,started_at,finished_at,input_payload&workflow_name=eq.onboarding_trace_verified&correlation_id=eq.$CorrelationId&order=created_at.desc&limit=1"
