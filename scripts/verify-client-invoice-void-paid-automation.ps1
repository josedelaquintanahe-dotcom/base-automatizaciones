...
function Get-PaidInvoice {
  param([string]$SupabaseBaseUrl, [string]$ClienteId)

  $paidUrl = "$SupabaseBaseUrl/rest/v1/facturas?select=id,cliente_id,mes,estado,total,created_at&cliente_id=eq.$ClienteId&estado=eq.pagada&order=created_at.desc&limit=1"
  $rows = @(Get-SupabaseRows -Url $paidUrl)

  if ($rows.Count -eq 0) {
    throw "No se encontro ninguna factura pagada para validar la anulacion."
  }
...
$templateKey = "client_invoice_void_paid"
$supabaseBaseUrl = $env:SUPABASE_URL.TrimEnd("/")
$paidInvoice = Get-PaidInvoice -SupabaseBaseUrl $supabaseBaseUrl -ClienteId $ClientId
$invoiceId = $paidInvoice.id
$invoiceMonth = $paidInvoice.mes
...
$executeResponse = Invoke-BackendJson -Method "POST" -Url $executeUrl -Headers @{
  ...
  scope = "invoice_void_paid"
  invoice_id = $invoiceId
  invoice_month = $invoiceMonth
  expected_status = "pagada"
  void_reason = "rollback_test_versioned"
}
...
if (
  ...
  $executionBody.resultado.workflow_name -ne "automation_client_invoice_void_paid" -or
...
$workflowUrl = "$supabaseBaseUrl/rest/v1/ejecuciones_workflows?select=workflow_name,status,correlation_id,input_payload,created_at,finished_at&workflow_name=eq.automation_client_invoice_void_paid&correlation_id=eq.$correlationId&order=created_at.desc&limit=1"
...
$rollbackResponse = Invoke-BackendJson -Method "POST" -Url $executeUrl -Headers @{
  ...
  scope = "invoice_void_paid_rollback"
  rollback_mode = $true
  invoice_id = $invoiceId
  invoice_month = $invoiceMonth
}
...
if (-not $rollbackResponse.ok -or -not $rollbackResponse.body.success) {
  throw "Fallo al ejecutar el rollback de la anulacion de factura pagada."
}
...
if (
  ...
  $rollbackBody.resultado.workflow_name -ne "automation_client_invoice_void_paid" -or
...
$rollbackWorkflowUrl = "$supabaseBaseUrl/rest/v1/ejecuciones_workflows?select=workflow_name,status,correlation_id,input_payload,created_at,finished_at&workflow_name=eq.automation_client_invoice_void_paid&correlation_id=eq.$rollbackCorrelationId&order=created_at.desc&limit=1"
...
for ($attempt = 1; $attempt -le $PollAttempts; $attempt++) {
  ...
  if (
    ...
    $invoiceAfterRollback.estado -eq "pagada"
  ) {
    break
  }
...
$result = [pscustomobject]@{
  target_invoice = [pscustomobject]@{
    invoice_id = $invoiceId
    invoice_month = $invoiceMonth
    previous_status = $paidInvoice.estado
...
if (
  ...
  $invoiceAfterRollback.estado -ne "pagada"
) {
  exit 1
}
