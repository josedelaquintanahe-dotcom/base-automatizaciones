...
const webhookTrigger = trigger({
  ...
  version: 2.1,
  config: {
    name: 'Webhook - onboarding_readiness_revalidated',
    parameters: {
      httpMethod: 'POST',
      path: 'onboarding_readiness_revalidated',
...
const buildReadinessContext = node({
  type: 'n8n-nodes-base.code',
  version: 2,
  config: {
    name: 'Code - Preparar readiness',
    parameters: {
      mode: 'runOnceForAllItems',
      language: 'javaScript',
      jsCode: `
...
const automationReadiness = onboardingPayload.automation_readiness ?? {};
const missingRequirements = Array.isArray(automationReadiness.missing_requirements)
  ? automationReadiness.missing_requirements
  : [];
const onboardingStatus = onboardingPayload.onboarding_status ?? null;
const ready = automationReadiness.ready === true;
const isConsistent = Boolean(
  source.valid &&
  matchedRow &&
  ready &&
  missingRequirements.length === 0 &&
  onboardingStatus === 'listo_para_automatizar'
);
...
return [
  {
    json: {
      ...
      result_summary: isConsistent
        ? 'Readiness del payload sigue consistente'
        : 'Readiness del payload no es consistente para continuar la secuencia',
      next_action: isConsistent
        ? 'Continuar con onboarding_credentials_metadata_checked'
        : 'Revisar automation_readiness y onboarding_status del evento origen',
      sanitized_result: {
        ready,
        ...
        fieldValues: [
          {
            ...
            fieldValue: 'onboarding_readiness_revalidated',
          },
          {
...
const verification = $('Code - Preparar readiness').first().json;
...
const checkReadiness = ifElse({
  version: 2.3,
  config: {
    name: 'IF - Readiness consistent',
    parameters: {
      conditions: {
        options: {
          caseSensitive: true,
...
const respondError = node({
  ...
  config: {
    ...
    parameters: {
      respondWith: 'json',
      responseBody: '={{ ({ accepted: false, workflow_name: "onboarding_readiness_revalidated", correlation_id: $("Code - Contexto update").first().json.correlation_id, result_summary: $("Code - Contexto update").first().json.result_summary, next_action: $("Code - Contexto update").first().json.next_action, sanitized_result: $("Code - Contexto update").first().json.sanitized_result }) }}',
      options: {
        responseCode: 422,
      },
    },
...
export default workflow('onboarding-readiness-revalidated-v1', 'onboarding_readiness_revalidated')
  .add(webhookTrigger)
  .to(validatePayload)
  .to(loadAutomationEvents)
  .to(buildReadinessContext)
  .to(insertExecution)
