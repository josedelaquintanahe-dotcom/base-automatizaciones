...
function Get-PendingInvoice {
  ...
  if ($rows.Count -eq 0) {
    throw "No se encontro ninguna factura pendiente para validar el marcado como pagada."
  }
...
$templateKey = "client_invoice_mark_paid"
...
$workflowUrl = "$supabaseBaseUrl/rest/v1/ejecuciones_workflows?select=workflow_name,status,correlation_id,input_payload,created_at,finished_at&workflow_name=eq.automation_client_invoice_mark_paid&correlation_id=eq.$correlationId&order=created_at.desc&limit=1"
...
$rollbackResponse = Invoke-BackendJson -Method "POST" -Url $executeUrl -Headers @{
  ...
  scope = "invoice_mark_paid_rollback"
  rollback_mode = $true
  invoice_id = $invoiceId
  invoice_month = $invoiceMonth
}
...
if (-not $rollbackResponse.ok -or -not $rollbackResponse.body.success) {
  throw "Fallo al ejecutar el rollback del marcado como pagada."
}
...
$rollbackWorkflowUrl = "$supabaseBaseUrl/rest/v1/ejecuciones_workflows?select=workflow_name,status,correlation_id,input_payload,created_at,finished_at&workflow_name=eq.automation_client_invoice_mark_paid&correlation_id=eq.$rollbackCorrelationId&order=created_at.desc&limit=1"
...
if (
  ...
  $invoiceRow.estado -ne "pagada" -or
